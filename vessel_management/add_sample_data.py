...
def add_sample_data():
    ...
    # Sample crew data with corrected ranks
    CREW_DATA = [
        {
            'name': 'John Smith',
            'rank': 'CAPTAIN',
        ...
        },
        {
            ...
            'passport_number': 'P3456789C',
            'seaman_book_number': 'SB3456789C',
        ...
        },
        {
            ...
            'passport_number': 'P4567890D',
            'seaman_book_number': 'SB4567890D',
    ...
    # Add vessels
    vessels_created = 0
    logger.info("Attempting to create vessels...")
    for vessel_data in VESSEL_DATA:
        try:
            ...
            if created:
                vessels_created += 1
                logger.info(f'Created vessel: {vessel.name} (IMO: {vessel.imo_number})')
            else:
                logger.info(f'Vessel already exists: {vessel.name} (IMO: {vessel.imo_number})')
        except Exception as e:
            logger.error(f'Error creating vessel {vessel_data["name"]}: {str(e)}')

    logger.info(f'Successfully created {vessels_created} vessels')
    ...
    for crew_data in CREW_DATA:
        try:
            ...
            if created:
                crew_created += 1
                logger.info(f'Created crew member: {crew.name} ({crew.rank})')
        except Exception as e:
            logger.error(f'Error creating crew member {crew_data["name"]}: {str(e)}')

    logger.info(f'Successfully created {crew_created} crew members')
            ...
            for cert_name, validity_years in certs.items():
                try:
                    ...
                    if created:
                        ...
                        logger.info(f'Created {cert_type} certificate "{cert_name}" for {crew.name}')
                except Exception as e:
                    logger.error(f'Error creating certificate {cert_name} for {crew.name}: {str(e)}')

    logger.info(f'Successfully created {crew_certificates_created} crew certificates')
        ...
        for cert_name, validity_years in certs.items():
            try:
                ...
                if created:
                    ...
                    logger.info(f'Created certificate type: {cert_name}')
            except Exception as e:
                logger.error(f'Error creating certificate type {cert_name}: {str(e)}')

    logger.info(f'Successfully created {vessel_cert_types_created} vessel certificate types')
            ...
            for cert_name, validity_years in certs.items():
                try:
                    ...
                    certificate, created = Certificate.objects.get_or_create(
                        vessel=vessel,
                        certificate_type=CertificateType.objects.get(name=cert_name),
                        defaults={
                            'certificate_number': f'{cert_type_name[:3]}-{vessel.id}-{vessel_certificates_created}',
                            'issue_date': issue_date,
                            'expiry_date': expiry_date,
                            'issuing_authority': 'Maritime Authority',
                            'document_file': None,
                    ...
                    if created:
                        ...
                        logger.info(f'Created {cert_name} certificate for {vessel.name}')
                except Exception as e:
                    logger.error(f'Error creating vessel certificate {cert_name} for {vessel.name}: {str(e)}')

    logger.info(f'Successfully created {vessel_certificates_created} vessel certificates')
